...
4. Use these subagents by role:
   - Use `Explore` for read-only inspection and research.
   - Use `general-purpose` for implementation, file changes, and other general execution. For a request that changes files, delegate implementation or final verification to `general-purpose`.
   - Use `Reviewer` only for read-only code review. Give it the relevant diff, paths, requirements, and test evidence. The main agent verifies findings and owns final decisions.
